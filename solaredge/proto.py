...
class Message(collections.namedtuple(
    ...
    def format(self, type_enum=None):
        type_desc = '0x%04x' % self.type
        if type_enum:
            try:
                type_desc = '%s (%s)' % (type_enum(self.type).name, type_desc)
            except ValueError:
                pass
        return (
            '<%s seq=0x%04x, addr_from=0x%04x, addr_to=0x%04x, type=%s, '
            'data=[%d bytes]>' % (
                self.__class__.__name__, self.seq, self.addr_from,
                self.addr_to, type_desc, len(self.data)))
...
def calculate_message_crc(msg):
    # The CRC covers the data and the big-endian versions of the
    # metadata.
    crc_header = struct.pack(
        '>HLLH', msg.seq, msg.addr_from, msg.addr_to, msg.type)
...
def encode_message(msg):
    assert len(msg.data) <= 0xffff
    header = struct.pack(
        '<4sHHHLLH', MAGIC, len(msg.data), ~len(msg.data) & 0xffff, msg.seq,
        msg.addr_from, msg.addr_to, msg.type)
    return header + msg.data + struct.pack('<H', calculate_message_crc(msg))
